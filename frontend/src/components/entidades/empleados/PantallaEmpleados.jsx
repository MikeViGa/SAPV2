...
import { obtenerEmpleadoNombreApi, obtenerEmpleadoApi, obtenerEmpleadosApi } from '../../api/EmpleadoApiService';
...
export default function PantallaEmpleados() {
     ...
     const { 
            ...
            salirModulo, 
            addSnackbar 
        } = useScreenCommon('Empleados', async (setRegistros, setCargando, addSnackbar) => {
            try {
                setCargando(true);
                const response = await obtenerEmpleadosApi();
        ...
        const obtenerOpciones = async (consulta) => {
            try {
                const respuesta = await obtenerEmpleadoNombreApi(consulta);
        ...
        const mostrarRegistros = async () => {
            ...
            try {
                if (idRegistro) {
                    const respuesta = await obtenerEmpleadoApi(idRegistro);
                    setRegistros([respuesta.data]);
                } else {
                    const respuesta = await obtenerEmpleadosApi();
                    setRegistros(respuesta.data);
                }
        ...
        return (
            <ScreenWrapper loading={cargando}>
                ...
                <Item>
                    <ListarEmpleados refrescar={mostrarRegistros} regs={registros} />
                </Item>
