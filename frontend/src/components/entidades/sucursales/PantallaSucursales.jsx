...
import { obtenerSucursalNombreApi, obtenerSucursalApi, obtenerSucursalesApi } from '../../api/SucursalApiService';
...
export default function PantallaSucursales() {
    ...
    const { 
            ...
            salirModulo, 
            addSnackbar 
        } = useScreenCommon('Sucursales', async (setRegistros, setCargando, addSnackbar) => {
            try {
                setCargando(true);
        ...
        const obtenerOpciones = async (consulta) => {
            try {
                const respuesta = await obtenerSucursalNombreApi(consulta);
        ...
        const mostrarRegistros = async () => {
            ...
            try {
                if (idRegistro) {
                    const respuesta = await obtenerSucursalApi(idRegistro);
                    setRegistros([respuesta.data]);
                } else {
                    const respuesta = await obtenerSucursalesApi();
                    setRegistros(respuesta.data);
                }
        ...
        return (
            <ScreenWrapper loading={cargando}>
                ...
                <Item>
                    <ListarSucursales refrescar={mostrarRegistros} regs={registros} />
                </Item>
