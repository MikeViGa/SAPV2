...
import { obtenerListaDePreciosDescripcionApi, obtenerListaDePreciosApi, obtenerListasDePreciosApi } from '../../api/ListaDePreciosApiService';
import ListarListasDePrecios from './ListarListasDePrecios';
...
export default function PantallaListasDePrecios() {
    ...
    const { 
            ...
            salirModulo, 
            addSnackbar 
        } = useScreenCommon('Listas de precios', async (setRegistros, setCargando, addSnackbar) => {
            try {
                setCargando(true);
                const response = await obtenerListasDePreciosApi();
        ...
        const obtenerOpciones = async (consulta) => {
            try {
                const respuesta = await obtenerListaDePreciosDescripcionApi(consulta);
        ...
        const mostrarRegistros = async () => {
            ...
            try {
                if (idRegistro) {
                    const respuesta = await obtenerListaDePreciosApi(idRegistro);
                    setRegistros([respuesta.data]);
                } else {
                    const respuesta = await obtenerListasDePreciosApi();
                    setRegistros(respuesta.data);
                }
        ...
        return (
            <ScreenWrapper loading={cargando}>
                ...
                <Item>
                    <ListarListasDePrecios refrescar={mostrarRegistros} regs={registros} />
                </Item>
