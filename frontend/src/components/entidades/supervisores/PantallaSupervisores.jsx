...
import { obtenerSupervisorNombreApi, obtenerSupervisorApi, obtenerSupervisoresApi } from '../../api/SupervisorApiService';
import ListarSupervisores from './ListarSupervisores';
...
export default function PantallaSupervisores() {
    ...
    const {
        ...
        salirModulo,
        addSnackbar
    } = useScreenCommon('Supervisores', async (setRegistros, setCargando, addSnackbar) => {
        try {
            setCargando(true);
            const response = await obtenerSupervisoresApi();
    ...
    const obtenerOpciones = async (consulta) => {
        try {
            const respuesta = await obtenerSupervisorNombreApi(consulta);
    ...
    const mostrarRegistros = async () => {
        ...
        try {
            if (idRegistro) {
                const respuesta = await obtenerSupervisorApi(idRegistro);
                setRegistros([respuesta.data]);
            } else {
                const respuesta = await obtenerSupervisoresApi();
                setRegistros(respuesta.data);
            }
    ...
    return (
        <ScreenWrapper loading={cargando}>
            ...
            <Item>
                <ListarSupervisores refrescar={mostrarRegistros} regs={registros} />
            </Item>
