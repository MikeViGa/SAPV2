...
import { eliminarJardinApi, obtenerReporteJardinApi } from "../../api/JardinApiService";
import StatusCell from "../../base/dashboard/elementos/StatusCell";
import FormularioJardin from "./FormularioJardin";
...
export default function ListarJardines({ refrescar, regs }) {

  const listadoHook = useListado(eliminarJardinApi);
  const { addSnackbar } = useSnackbar();

  const descargarReporte = async () => {
    listadoHook.setCargando(true);
    try {
      const respuesta = await obtenerReporteJardinApi();
