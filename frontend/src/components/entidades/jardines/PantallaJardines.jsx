...
import { obtenerJardinDescripcionApi, obtenerJardinApi, obtenerJardinesApi } from '../../api/JardinApiService';
import ListarJardines from './ListarJardines';
...
export default function PantallaJardines() {
    ...
    const { 
            ...
            salirModulo, 
            addSnackbar 
        } = useScreenCommon('Jardines', async (setRegistros, setCargando, addSnackbar) => {
            try {
                setCargando(true);
                const response = await obtenerJardinesApi();
        ...
        const obtenerOpciones = async (consulta) => {
            try {
                const respuesta = await obtenerJardinDescripcionApi(consulta);
        ...
        const mostrarRegistros = async () => {
            ...
            try {
                if (idRegistro) {
                    const respuesta = await obtenerJardinApi(idRegistro);
                    setRegistros([respuesta.data]);
                } else {
                    const respuesta = await obtenerJardinesApi();
                    setRegistros(respuesta.data);
                }
        ...
        return (
            <ScreenWrapper loading={cargando}>
                ...
                <Item>
                    <ListarJardines refrescar={mostrarRegistros} regs={registros} />
                </Item>
