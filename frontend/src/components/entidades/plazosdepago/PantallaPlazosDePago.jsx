...
import { obtenerPlazoDePagoDescripcionApi, obtenerPlazoDePagoApi, obtenerPlazosDePagoApi } from '../../api/PlazosDePagoApiService';
import ListarPlazosDePago from './ListarPlazosDePago';
...
export default function PantallaPlazosDePago() {
    ...
    const { 
            ...
            salirModulo, 
            addSnackbar 
        } = useScreenCommon('PlazosDePago', async (setRegistros, setCargando, addSnackbar) => {
            try {
                setCargando(true);
                const response = await obtenerPlazosDePagoApi();
        ...
        const obtenerOpciones = async (consulta) => {
            try {
                const respuesta = await obtenerPlazoDePagoDescripcionApi(consulta);
        ...
        const mostrarRegistros = async () => {
            ...
            try {
                if (idRegistro) {
                    const respuesta = await obtenerPlazoDePagoApi(idRegistro);
                    setRegistros([respuesta.data]);
                } else {
                    const respuesta = await obtenerPlazosDePagoApi();
                    setRegistros(respuesta.data);
                }
        ...
        return (
            <ScreenWrapper loading={cargando}>
                <Item>
                    <ListarPlazosDePago refrescar={mostrarRegistros} regs={registros} />
                </Item>
