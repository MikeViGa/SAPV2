...
import { actualizarPlazoDePagoApi, crearPlazoDePagoApi, } from '../../api/PlazosDePagoApiService';
...
export default function FormularioPlazoDePago({ modo, registro, open, onClose, refrescar }) {
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values, { setSubmitting }) => {
            try {
                ...
                const response = (modo === "editar" ? actualizarPlazoDePagoApi(formData.id, formData) : crearPlazoDePagoApi(formData))
                    .then(response => {
                ...
                console.log(`Error inesperado ${values.id ? ' actualizando' : ' creando'} plazodepago: ${err.message}`, "error");
            } finally {
                setSubmitting(false);
                setLoading(false);
            }
            ...
            <DialogTitle sx={{ bgcolor: '#1976d2', color: '#fff', py: 1, px: 2 }}>
                <Box display="flex" justifyContent="space-between" alignItems="center">
                    <Typography variant="h6" sx={{ color: 'inherit' }}>
                        {modo === 'editar' ? 'Editar plazo de pago' : 'Crear plazo de pago'}
                    </Typography>
