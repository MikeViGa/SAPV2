...
import { eliminarPlazoDePagoApi, obtenerReportePlazoDePagoApi } from "../../api/PlazosDePagoApiService";
import StatusCell from "../../base/dashboard/elementos/StatusCell";
import FormularioPlazoDePago from './FormularioPlazoDePago';
...
export default function ListarPlazosDePago({ refrescar, regs }) {

  const listadoHook = useListado(eliminarPlazoDePagoApi);
  const { addSnackbar } = useSnackbar();

  const descargarReporte = async () => {
    listadoHook.setCargando(true);
    try {
      const respuesta = await obtenerReportePlazoDePagoApi();
